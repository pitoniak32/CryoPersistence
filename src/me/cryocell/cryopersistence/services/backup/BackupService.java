...
public class BackupService {
    ...
    public BackupService(ConfigService configService, CryoPersistence plugin) {
        ...
        if(this.configService.getBackupConfig().getAutoBackupIntervalTicks() > 0) this.scheduleBackupTask();
    }
    ...
    public void createNewBackup(){
        ...
        new Thread(() -> {
            MessageUtils.broadcastMessage(
                    plugin.getServer(),
                    ChatColor.DARK_GRAY + this.configService.getTag() + "server backup starting"
            );
